...
class ConditionsTest < Minitest::Test
  ...
  def test_ten_day_forecast_text
   assert Forecast.new("72401").forecast_all.include?"Clear skies with a few passing clouds. Low 56F. Winds light and variable."
 end
  ...
  def test_sunrise_time
    assert_equal "6:49", Astronomy.new("72401").sunrise
  end

  def test_sunrise_time
    assert_equal "19:01", Astronomy.new("72401").sunset
  end
